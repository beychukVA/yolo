...
export const MarketCloseAlert = React.memo(({ assetName, openTimeUtc, closeTimeUtc }) => {
  return (
    <AlertWrapper>
      <ModalPad>
        <AlertIcon />
        <ModalTitle> Market is currently closed</ModalTitle>
        <ModalParagrah>
          {`${assetName} market hours are ${openTimeUtc} - ${closeTimeUtc} UTC time from Monday to Friday. Please return at the open.`}
        </ModalParagrah>
      </ModalPad>
    </AlertWrapper>
  )
