...
const TxItem = ({ status, roundNumber, timestamp, hashValue, usdAmount, yoloAmount }) => {
  ...
  const { explorerTxTemplate } = useSelector(selectChainInfo()) || {}
  const hashLink = useMemo(
    () => (explorerTxTemplate ? explorerTxTemplate.replace('[hashAddress]', hashValue) : ''),
    [explorerTxTemplate, hashValue]
  )
      ...
      <TxListWrapper>
        {bidTxList.map((tx, index) => (
          <TxItem
            status={tx.status}
            key={index}
            roundNumber={tx.txParams.bidRoundIndex}
            timestamp={formatTimeFromNow(tx.timestamp)}
            hashValue={tx.hash}
            usdAmount={toFiatAmount(tx.txParams.amount, tx.txParams.currency)}
            yoloAmount={toCryptoAmount(tx.txParams.amount, tx.txParams.currency)}
          />
        ))}
      </TxListWrapper>
    </BidsWrapper>
