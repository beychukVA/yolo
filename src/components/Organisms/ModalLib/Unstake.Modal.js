...
import { useYlpUnstake } from 'hooks/ylp/useYlpUnstake'
...
export const UnstakeModal = ({ closeModal }) => {
  const { account } = useUser('wallet')
  const amountInputRef = useRef()
  const [amount, setAmount] = useState('')

  const { unstakeYLP, status, hasStatus } = useYlpUnstake()
  ...
  }, [isYlpApproved, closeModal, ylpStakedBalance])
  ...
  return (
    <ModalOverlay>
      <ModalBox isError={status.id === ASYNC_STATUS_ID.ERROR}>
        ...
        <ModalHeading>
          <Title> Unstake YLP tokens </Title>
        </ModalHeading>
        <UnstakeForm onSubmit={(e) => e.preventDefault()}>
          <fieldset>
            ...
            <label htmlFor='input_amount_entry'>Amount to unstake</label>
            <InputWrap>
              <AmountInput
                ref={amountInputRef}
                type='number'
                ...
                maxLength='9'
                value={amount}
                isError={!ylpStakedBalance || ylpStakedBalance === 0 || amount > ylpStakedBalance}
                onChange={(e) => setAmount(e.target.value)}
              ></AmountInput>
            </InputWrap>
          </fieldset>
          <InputError isError={!ylpStakedBalance || ylpStakedBalance === 0 || amount > ylpStakedBalance}>
            Insufficient Balance
          </InputError>
          <AmountSuggestions>
            {SUGGESTED_PERCENTS.map((percent, index) => {
              const amount = (ylpStakedBalance / 100) * percent
              return (
                <button id={`amountPreset-${amount}`} key={index} onClick={() => setAmount(amount)}>
          ...
          <ButtonsContainers>
            <UnstakeButton
              ...
              disabled={
                !ylpStakedBalance || ylpStakedBalance === 0 || !amount || amount === 0 || amount > ylpStakedBalance
              }
            >
              <ContentSwitcherByState
                ...
                stateObject={{
                  ...
                  [ASYNC_STATUS_ID.ERROR]: 'Retry unstake',
                  default: <SingleDataLoader loading={status.id === ASYNC_STATUS_ID.PENDING} data={'Unstake'} />
                }}
              />
            </UnstakeButton>
...
const ButtonsContainers = styled.div`
  ...
  padding: 10px 0 0 0;
`
const UnstakeButton = styled(DashboardButton)`
  height: 43px;
  min-width: 65%;
`
const CloseButton = styled(UnstakeButton)`
