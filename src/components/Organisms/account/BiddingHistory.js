...
import { Header_1 } from './sharedComponent'
            ...
            {tableData.tableData.map((rowData, index) => {
              return (
                <React.Fragment key={index}>
                  ...
                  <TotalYoloEarningsCell key={`b1-${index}`}>{rowData[1]}</TotalYoloEarningsCell>
                  <TotalYoloEarningsCell key={`b2-${index}`}>{rowData[2]}</TotalYoloEarningsCell>
                  ...
                  <BidDirectionCell key={`e-${index}`}>
                    <IconArrow
                      masking
                      rotate={rowData[5] === 'up' ? 'down' : 'up'}
                      isUp={rowData[5] === 'up'}
                    ></IconArrow>
                    {capitalizeFirst(rowData[5])}
                  </BidDirectionCell>
                  <RoundResultCell>{rowData[6]}</RoundResultCell>
                </React.Fragment>
              )
            })}
          </HistoryGrid>
