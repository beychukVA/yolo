...
const spLineTypeD = (pointsArray, sharp = false) => {
  ...
  const grouped = pointsArray.map((pt, idx) => {
    ...
    if (idx === pointsArray.length - 1) return '' //return `L100,${pt[1]}`
...
export const CurrentPath = ({ gamePlotData, tailLine }) => {
  const theme = useContext(ThemeContext)
  const { currentPrice, strikePrice, pathValues, lastValue, yDomain, xDomain } = gamePlotData
  ...
  const getCoordinateY = useCallback(() => {
    return linearInterpolator([yDomain.min, yDomain.max], yRange)
  }, [yDomain?.min, yDomain?.max])

  const getCoordinateX = useCallback(
    () => linearInterpolator([xDomain.min, xDomain.max], xRange),
  ...
  const makeLinePathDefinition = useCallback(() => {
    ...
    pathValues.shift()
    const pointsArray = [...pathValues, lastValue].map((point, idx) => {
      const x2 = getCoordinateX()(point.x)
      const y2 = getCoordinateY()(point.y)
      return [x2, y2]
    })
    const tailPoint = `L100,${getCoordinateY()(lastValue)}`
    const dSp = [...spLineTypeD(pointsArray), tailLine ? tailPoint : '']
    return dSp
  }, [getCoordinateX, getCoordinateY, pathValues, lastValue, tailLine])
  ...
  const canRenderPath = useMemo(() => {
    if (isEmpty(gamePlotData)) return false
    if (!gamePlotData?.lastValue?.x) return false
    if (!gamePlotData?.lastValue?.y) return false
    return true
  }, [gamePlotData])

  return canRenderPath ? (
