...
export const G24hrLiveCard = ({ className, gId, cardRoundOffset }) => {
  ...
  const { LIVE_ROUND_FLASH_TIME, gameTypeLabel, liveTileBackgroundColor } = getGameParameters(gId)
        ...
        <div className='q1'>
          <div className='block_time_wrap'>
            ...
            <div id='real_time_value' className='test'>
              {formatDurationMilliseconds(msTimeLeft + timeOffsetMs, 'hh:mm:ss')}
            </div>
          </div>
        </div>
        <div className='q2'>
          <strong>{gameTypeLabel}</strong> LIVE ROUND {ownCardRoundIdx}
        </div>
        <div className='q3'>
