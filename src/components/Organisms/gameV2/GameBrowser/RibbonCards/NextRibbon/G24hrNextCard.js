...
import { currencyFormatter, formatDurationMilliseconds, formatTimeStamp } from 'utils'
...
export const G24hrNextCard = ({ className, gId, cardRoundOffset }) => {
  ...
  const { gamePeriod, msStartsIn } = useGameProgress(gId)
  const nextMsTimeLeft = msStartsIn + (cardRoundOffset - 1) * gamePeriod
  // Memoize some data
  ...
  return (
    <NextCardStyled isActive={isActive} onClick={onClickNext}>
      <div className={`game next ${isActive ? 'active' : ''}`} id='ribbonCardBase'>
        <div className='q1'>
          <div className='game_status'>
            ...
            <div className='gametime'>
              <div className='game_block_value'>
                {formatDurationMilliseconds(nextMsTimeLeft + timeOffsetMs, 'hh:mm:ss')}
              </div>
            </div>
          </div>
        </div>
        <div className='q2'>
          <SingleContentToggle
            toggle={liveRoundData.status === 'live' && cardRoundOffset === 1}
            falseContent={
