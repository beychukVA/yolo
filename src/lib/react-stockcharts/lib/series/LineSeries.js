...
class LineSeries extends Component {
  ...
  renderSVG(moreProps) {
    const { yAccessor, strokeOpacity, strokeWidth, hoverStrokeWidth, defined, strokeDasharray } = this.props
    const { connectNulls } = this.props
    const { interpolation, style, stroke } = this.props
    ...
    return (
      <>
        <defs>
          <filter id='glowPath'>
            <feFlood result='flood' floodColor={stroke} floodOpacity='1'></feFlood>
            <feComposite in='flood' in2='SourceGraphic' operator='in' result='color-out'></feComposite>
            <feGaussianBlur result='color-out' stdDeviation='6' in='mask'></feGaussianBlur>
        ...
        <path
          ...
          strokeDasharray={getStrokeDasharray(strokeDasharray) || strokeDasharray}
          fill={fill}
          // filter={`url(#glowPath)`}
        />
      </>
...
LineSeries.propTypes = {
  ...
  strokeDasharray: PropTypes.oneOf(strokeDashTypes, Array),
