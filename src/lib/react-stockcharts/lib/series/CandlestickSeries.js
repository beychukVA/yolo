...
import { hexToRGBA, isDefined, functor, plotDataLengthBarWidth, head } from 'react-stockcharts/lib/utils'
...
class CandlestickSeries extends Component {
  ...
  renderSVG(moreProps) {
    ...
    console.log('ACZ plotData -->', plotData)
...
function getCandlesSVG(props, candleData) {
  ...
  const candles = candleData.map((d, idx) => {
    if (d.width <= 1)
      return <line className={d.className} key={idx} x1={d.x} y1={d.y} x2={d.x} y2={d.y + d.height} stroke={d.fill} />
    else if (d.height === 0)
